...
hs <- makeParamSet(
  ...
  makeNumericParam("feature_fraction", lower = 0.2, upper = 0.80),
  #makeNumericParam("lambda_l1", lower= 0, upper = 800),
  makeNumericParam("lambda_l2", lower= 0, upper = 800),
  #makeNumericParam("min_gain_to_split", lower = 0, upper = 50),
  makeIntegerParam("min_data_in_leaf", lower = 1500, upper = 8000L),
  makeIntegerParam("max_depth", lower = 12, upper = 18),
  #makeIntegerParam("num_leaves", lower = 16L, upper = 1024L),
  #makeIntegerParam("num_iterations", lower = 5000, upper = 12000),
  makeIntegerParam("envios", lower = 9500L, upper = 15000L),
  makeIntegerParam("max_bin", lower = 30, upper = 300),
  makeNumericParam("bagging_fraction", lower= .30, upper = 1)
)
...
EstimarGanancia_lightgbm <- function(x) {
  ...
  param_basicos <- list(
    objective = "binary",
    metric = "custom",
    #boosting = "gbdt", #better for accuracy - implements drop outs of later trees.
    first_metric_only = TRUE,
    boost_from_average = TRUE, # dejo asi
    ...
    #force_col_wise = TRUE,
    seed = ksemilla_azar1,
    bagging_freq = 10 # tomar otra muestra cada 10 arboles.
  )

  # el parametro discolo, que depende de otro
  param_variable <- list(
    num_leaves = 
      as.integer(2^(x$max_depth -1)),
